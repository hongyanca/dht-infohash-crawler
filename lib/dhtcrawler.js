...
class DHTCrawler extends events.EventEmitter {
  constructor(options = { address: '0.0.0.0', port: 6881, kbucketSize: 512, name: 'crawler' }) {
    ...
    this._recentInfohashes = new InfohashQueue(256);
  ...
  onUDPMessage (data, peerAddress) {
    try {
      let message;
      try { message = bencode.decode(data); } catch (err) {}
      
      /* 
       * It is not in the specification on BitTorrent.org, but BT clients add an extra key to the 
       * DHT messages, indicating the Version/Client. http://getright.com/torrentdev.html
       * As a crawler, we can just ignore this message.
       */
  ...
  listen () {
    ...
    setInterval(() => {
      // console.log(`${this._name} kbucket size: ${this._kbucket.size}`);
      if (this._kbucket.size < 64) this.joinDHTNetwork();
    }, 3000);
    setInterval(() => {      
      this.broadcastSelf();
    }, 1000);
